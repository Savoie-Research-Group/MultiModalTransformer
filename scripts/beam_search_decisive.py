#This .py file is used to calculate the decisiveness of token in the decoded SMILES string

import torch
import numpy as np
import math

def output_tensor_to_prob(model, dec_out_tensor, num_classes, beam_width, first_flag):
    if first_flag:
        prob_tensor = model.ded_classifier(dec_out_tensor)
...
def beam_search(model, source, beam_width, src_maxlen, tgt_maxlen, target_start_token_idx, target_end_token_idx):
    ...
    for _ in range(bs):
        ...
        dec_out_all = model.src_classifier(dec_out_src)
        ...
        p_numpy = output_tensor_to_prob(model,dec_out_final,num_classes,beam_width,first_flag=True)
        p_numpy_0src = output_tensor_to_prob(model,dec_out_final_0src,num_classes,beam_width,first_flag=True)
        p_numpy_0ms = output_tensor_to_prob(model,dec_out_final_0ms,num_classes,beam_width,first_flag=True)
        p_numpy_0ir = output_tensor_to_prob(model,dec_out_final_0ir,num_classes,beam_width,first_flag=True)
        p_numpy_0nmr = output_tensor_to_prob(model,dec_out_final_0nmr,num_classes,beam_width,first_flag=True)
        ...
        for i in range(tgt_maxlen-2):
            ...
            p_numpy = output_tensor_to_prob(model,dec_out_final,num_classes,beam_width,first_flag=False)
            p_numpy_0src = output_tensor_to_prob(model,dec_out_final_0src,num_classes,beam_width,first_flag=False)
            p_numpy_0ms = output_tensor_to_prob(model,dec_out_final_0ms,num_classes,beam_width,first_flag=False)
            p_numpy_0ir = output_tensor_to_prob(model,dec_out_final_0ir,num_classes,beam_width,first_flag=False)
            p_numpy_0nmr = output_tensor_to_prob(model,dec_out_final_0nmr,num_classes,beam_width,first_flag=False)
